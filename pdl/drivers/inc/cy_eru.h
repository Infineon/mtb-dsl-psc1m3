 ...
 * @addtogroup ERU
 * @brief Event Request Unit (ERU) driver for the PSC1M microcontroller family.
 *
 ...
 * This section of the LLD provides the configuration structure Cy_ERU_ETL_CONFIG_t and the initialization function
 * Cy_ERU_ETL_Init().\n
 * It can be used to:
 * -# Select one out of two inputs (A and B). For each of these two inputs, a vector of 4 possible signals is available.
 *   (Cy_ERU_ETL_SetSource())
 * -# Logically combine the two input signals to a common trigger. (Cy_ERU_ETL_SetSource())
 * -# Define the transition (edge selection, or by software) that leads to a trigger event and can also store this status.
 *   (Cy_ERU_ETL_SetEdgeDetection() and Cy_ERU_ETL_SetStatusFlag())
 * -# Distribute the events and status flags to the output channels. (Cy_ERU_ETL_EnableOutputTrigger())
 *
 * \par Output gating unit (ERU_OGU):
 * This section of the LLD provides the provides the configuration structure Cy_ERU_OGU_CONFIG_t and the initialization
 * function Cy_ERU_ETL_OGU_Init().
 * It can be used to:
 * -# Combine the trigger events and status information and gates the output depending on a gating signal.
 *   (Cy_ERU_OGU_EnablePatternDetection(), Cy_ERU_OGU_DisablePeripheralTrigger(), Cy_ERU_OGU_SetServiceRequestMode())
 * @{
 */
...
#if defined(ERU0)
#define CY_ERU0 ((Cy_ERU_t *) ERU0_BASE) /**< ERU module 0 */
#endif

#if defined(ERU1)
#define CY_ERU1 ((Cy_ERU_t *) ERU1_BASE) /**< ERU module 1 */
#endif

#include "cy_eru_map.h"

#if defined(CY_ERU0) && defined(CY_ERU1)
#define CY_ERU_CHECK_MODULE_PTR(PTR)  (((PTR)== CY_ERU0) | ((PTR)== CY_ERU1))
#elif defined(CY_ERU0)
#define CY_ERU_CHECK_MODULE_PTR(PTR)  (((PTR)== CY_ERU0))
#elif defined(CY_ERU1)
#define CY_ERU_CHECK_MODULE_PTR(PTR)  (((PTR)== CY_ERU1))
#endif
...
*/
typedef enum CY_ERU_ETL_INPUT_A
{
  CY_ERU_ETL_INPUT_A0 = 0x0U, /**< input A0 is selected */
  CY_ERU_ETL_INPUT_A1 = 0x1U, /**< input A1 is selected */
  CY_ERU_ETL_INPUT_A2 = 0x2U, /**< input A2 is selected */
  CY_ERU_ETL_INPUT_A3 = 0x3U  /**< input A3 is selected */
} Cy_ERU_ETL_INPUT_A_t;
 ...
 */
typedef enum CY_ERU_ETL_INPUT_B
{
  CY_ERU_ETL_INPUT_B0 = 0x0U, /**< input B0 is selected */
  CY_ERU_ETL_INPUT_B1 = 0x1U, /**< input B1 is selected */
  CY_ERU_ETL_INPUT_B2 = 0x2U, /**< input B2 is selected */
  CY_ERU_ETL_INPUT_B3 = 0x3U  /**< input B3 is selected */
} Cy_ERU_ETL_INPUT_B_t;
 ...
 */
typedef enum CY_ERU_ETL_SOURCE
{
  CY_ERU_ETL_SOURCE_A = 0x0U,              /**< select (A) path as a event source */
  CY_ERU_ETL_SOURCE_B = 0x1U,              /**< select (B) path as a event source */
  CY_ERU_ETL_SOURCE_A_OR_B = 0x2U,         /**< select (A <b>OR</b> B) path as a event source */
  CY_ERU_ETL_SOURCE_A_AND_B = 0x3U,        /**< select (A <b>AND</b> B) path as a event source */
  CY_ERU_ETL_SOURCE_NOT_A = 0x4U,          /**< select (inverted A) path as a event source */
  CY_ERU_ETL_SOURCE_NOT_A_OR_B = 0x6U,     /**< select (inverted A <b>OR</b> B) path as a event source */
  CY_ERU_ETL_SOURCE_NOT_A_AND_B = 0x7U,    /**< select (inverted A <b>AND</b> B) path as a event source */
  CY_ERU_ETL_SOURCE_NOT_B = 0x9U,          /**< select (inverted B) path as a event source */
  CY_ERU_ETL_SOURCE_A_OR_NOT_B = 0xaU,     /**< select (A <b>OR</b> inverted B) path as a event source */
  CY_ERU_ETL_SOURCE_A_AND_NOT_B = 0xbU,    /**< select (A <b>AND</b> inverted B) path as a event source */
  CY_ERU_ETL_SOURCE_NOT_A_OR_NOT_B = 0xeU, /**< select (inverted A <b>OR</b> inverted B) path as a event
                                                           source */
  CY_ERU_ETL_SOURCE_NOT_A_AND_NOT_B = 0xfU /**< select (inverted A <b>AND</b> inverted B) path as a event
                                                           source */
} Cy_ERU_ETL_SOURCE_t;
 ...
 */
typedef enum CY_ERU_ETL_EDGE_DETECTION
{
  CY_ERU_ETL_EDGE_DETECTION_DISABLED = 0U, /**< no event enabled */
  CY_ERU_ETL_EDGE_DETECTION_RISING = 1U,   /**< detection of rising edge generates the event */
  CY_ERU_ETL_EDGE_DETECTION_FALLING = 2U,  /**< detection of falling edge generates the event */
  CY_ERU_ETL_EDGE_DETECTION_BOTH = 3U      /**< detection of either edges generates the event */
} Cy_ERU_ETL_EDGE_DETECTION_t;

/**
 * Defines Output Channel of OGUy(Output gating unit y = [0 to 3]) to be mapped by the trigger pulse generated by
 * ETLx(Event Trigger Logic, x = [0 to 3]) unit.
 * @note Generation of output trigger pulse need to be enabled @ref Cy_ERU_OGU_PERIPHERAL_TRIGGER_t
 * @image html "eru_connection_matrix.png" "ERU_ETL ERU_OGU Connection matrix"
 */
typedef enum CY_ERU_ETL_OUTPUT_TRIGGER_CHANNEL
{
  CY_ERU_ETL_OUTPUT_TRIGGER_CHANNEL0 = 0U, /**< Event from input ETLx triggers output OGU0 */
  CY_ERU_ETL_OUTPUT_TRIGGER_CHANNEL1 = 1U, /**< Event from input ETLx triggers output OGU1 */
  CY_ERU_ETL_OUTPUT_TRIGGER_CHANNEL2 = 2U, /**< Event from input ETLx triggers output OGU2 */
  CY_ERU_ETL_OUTPUT_TRIGGER_CHANNEL3 = 3U, /**< Event from input ETLx triggers output OGU3 */
} Cy_ERU_ETL_OUTPUT_TRIGGER_CHANNEL_t;

/**
 * Defines generation of the trigger pulse by ETLx(Event Trigger Logic, x = [0 to 3]) unit.
 * Use type Cy_ERU_ETL_OUTPUT_TRIGGER_t for this enum.
 */
typedef enum CY_ERU_ETL_OUTPUT_TRIGGER
{
  CY_ERU_ETL_OUTPUT_TRIGGER_DISABLED = 0U, /**< trigger pulse generation disabled */
  CY_ERU_ETL_OUTPUT_TRIGGER_ENABLED = 1U   /**< trigger pulse generation enabled */
} Cy_ERU_ETL_OUTPUT_TRIGGER_t;

/**
 * Defines status flag reset mode generated by ETLx(Event Trigger Logic, x = [0 to 3]) unit.
 * Use type Cy_ERU_ETL_STATUS_FLAG_MODE_t for this enum.
 */
typedef enum CY_ERU_ETL_STATUS_FLAG_MODE
{
  CY_ERU_ETL_STATUS_FLAG_MODE_SWCTRL = 0U, /**< Status flag is in sticky mode. Retain the same state until
                                                 cleared by software. In case of pattern match this mode
                                                 is used. */
  CY_ERU_ETL_STATUS_FLAG_MODE_HWCTRL = 1U  /**< Status flag is in non-sticky mode. Automatically cleared by
...
} Cy_ERU_ETL_STATUS_FLAG_MODE_t;

/**
 * Defines pattern detection feature to be enabled or not in OGUy(Output gating unit, y = [0 to 3]).
 *
 */
typedef enum CY_ERU_OGU_PATTERN_DETECTION
{
  CY_ERU_OGU_PATTERN_DETECTION_DISABLED = 0U, /**< Pattern match is disabled */
  CY_ERU_OGU_PATTERN_DETECTION_ENABLED = 1U   /**< Pattern match is enabled, the selected status flags of
                                                    ETLx(Event Trigger Logic, x = [0 to 3]) unit, are
                                                    used in pattern detection. */
} Cy_ERU_OGU_PATTERN_DETECTION_t;
 ...
 */
typedef enum CY_ERU_OGU_PATTERN_DETECTION_INPUT
{
  CY_ERU_OGU_PATTERN_DETECTION_INPUT0 = 1U, /**< Status flag ETL0, participating in pattern match */
  CY_ERU_OGU_PATTERN_DETECTION_INPUT1 = 2U, /**< Status flag ETL1, participating in pattern match */
  CY_ERU_OGU_PATTERN_DETECTION_INPUT2 = 4U, /**< Status flag ETL0, participating in pattern match */
  CY_ERU_OGU_PATTERN_DETECTION_INPUT3 = 8U  /**< Status flag ETL0, participating in pattern match */
} Cy_ERU_OGU_PATTERN_DETECTION_INPUT_t;
 ...
 */
typedef enum CY_ERU_OGU_PERIPHERAL_TRIGGER
{
  CY_ERU_OGU_PERIPHERAL_TRIGGER1 = 1U, /**< OGUy1 signal is mapped for event generation */
  CY_ERU_OGU_PERIPHERAL_TRIGGER2 = 2U, /**< OGUy2 signal is mapped for event generation */
  CY_ERU_OGU_PERIPHERAL_TRIGGER3 = 3U  /**< OGUy3 signal is mapped for event generation */
} Cy_ERU_OGU_PERIPHERAL_TRIGGER_t;
 ...
 */
typedef enum CY_ERU_OGU_SERVICE_REQUEST
{
  CY_ERU_OGU_SERVICE_REQUEST_DISABLED = 0U,                      /**< Service request blocked, ERUx_GOUTy = 0 */
  CY_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER = 1U,                    /**< Service request generated enabled, ERUx_GOUTy = 1 */
  CY_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER_AND_PATTERN_MATCH = 2U,  /**< Service request generated on trigger
                                                                       event and input pattern match,
                                                                       ERUx_GOUTy = ~pattern matching result*/
  CY_ERU_OGU_SERVICE_REQUEST_ON_TRIGGER_AND_PATTERN_MISMATCH = 3U/**< Service request generated on trigger
                                                                       event and input pattern mismatch,
                                                                       ERUx_GOUTy = pattern matching result*/
} Cy_ERU_OGU_SERVICE_REQUEST_t;
 ...
 */
typedef struct CY_ERU_ETL_CONFIG
{
  union
  {
    ...
    struct
    {
      uint32_t input_a: 2; /**< Configures input A. Refer @ref Cy_ERU_ETL_INPUT_A_t for valid values */
      uint32_t input_b: 2; /**< Configures input B. Refer @ref Cy_ERU_ETL_INPUT_B_t for valid values */
      uint32_t : 28;
    };
  };
 ...
 */
typedef union CY_ERU_OGU_CONFIG
{
  uint32_t raw;
  ...
  struct
  {
    uint32_t peripheral_trigger: 2;        /**< peripheral trigger(ISS) input selection.
                                                  Refer @ref Cy_ERU_OGU_PERIPHERAL_TRIGGER_t for valid values. */
    ...
    uint32_t service_request: 2;          /**< Gating(GP) on service request generation for pattern detection result.
                                                 Refer @ref Cy_ERU_OGU_SERVICE_REQUEST_t for valid values. */
    uint32_t : 6;
    uint32_t pattern_detection_input: 4;  /**< Enable input for the pattern detection(IPENx, x = [0 to 3]).
                                                 Refer @ref Cy_ERU_OGU_PATTERN_DETECTION_INPUT_t for valid values.
...
/**
 * @param eru A constant pointer to Cy_ERU_t, pointing to the ERU base address
 *
 * @return None
 *
 ...
 * \par
 * This API is called by Cy_ERU_ETL_Init() or Cy_ERU_OGU_Init() and therefore no need to call it explicitly during
 * initialization sequence. Call this API to enable ERU1 module once again if the module is disabled by calling
 * Cy_ERU_Disable(). For ERU0 module clock gating and reset features are not available.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_Init(), Cy_ERU_OGU_Init(), Cy_ERU_Disable().
 */
 ...
 *               Refer data structure Cy_ERU_ETL_CONFIG_t for detail.
 *
 * @return None
 *
 * <b>Description:</b><br>
 ...
 *
 * Invokes Cy_ERU_Enable() to enable \a eru module clock. Then configures
 * <ul>
 ...
 * </ul>.
 */
void Cy_ERU_ETL_Init(Cy_ERU_t *const eru, const uint8_t channel, const Cy_ERU_ETL_CONFIG_t *const config);
 ...
 *                Refer Cy_ERU_ETL_INPUT_A_t for valid value or cy_eru_map.h file where the mapping of
 ...
 *                Refer Cy_ERU_ETL_INPUT_B_t for valid value or cy_eru_map.h file where the mapping of
 *                the input is done based on selected signal.\n
 ...
 * \par
 * These values are set during initialization in Cy_ERU_ETL_Init(). Call this to change the input, as needed later in
 ...
 *               Refer @ref Cy_ERU_ETL_SOURCE_t enum for valid input values.
 *
 * @return None
 *
 * \par<b>Description:</b><br>
 ...
 * x = [0 to 3]) for further action. These values are set during initialization in Cy_ERU_ETL_Init(). Call this to
 ...
 *                       Refer @ref Cy_ERU_ETL_EDGE_DETECTION_t enum for valid values.
 *
 * @return None
 *
 * \par<b>Description:</b><br>
 ...
 * initialization in Cy_ERU_ETL_Init(). Call this to change the trigger edge, as needed later in the program.
 */
 ...
 *                Range : [0 to 3]
 *
 * @return Cy_ERU_ETL_EDGE_DETECTION_t indicate which egde/s is configured for event generation
 *
 * \par<b>Description:</b><br>
...
Cy_ERU_ETL_EDGE_DETECTION_t Cy_ERU_ETL_GetEdgeDetection(Cy_ERU_t *const eru,
    const uint8_t channel);
/**
 * @param eru A constant pointer to Cy_ERU_t, pointing to the ERU base address.
 * @param channel ERU_ETLx(Event trigger logic unit) channel.
 ...
 * \par
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_ClearStatusFlag(), Cy_ERU_ETL_GetStatusFlag()
 */
__STATIC_INLINE void Cy_ERU_ETL_SetStatusFlag(Cy_ERU_t *const eru, const uint8_t channel)
{
 ...
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_SetStatusFlag(), Cy_ERU_ETL_GetStatusFlag()
 */
__STATIC_INLINE void Cy_ERU_ETL_ClearStatusFlag(Cy_ERU_t *const eru, const uint8_t channel)
{
  CY_ASSERT(CY_ERU_CHECK_MODULE_PTR(eru));
  CY_ASSERT((channel < 4U));
 ...
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_SetStatusFlag(), Cy_ERU_ETL_ClearStatusFlag()
 */
__STATIC_INLINE uint32_t Cy_ERU_ETL_GetStatusFlag(Cy_ERU_t *const eru, const uint8_t channel)
{
  CY_ASSERT(CY_ERU_CHECK_MODULE_PTR(eru));
  CY_ASSERT((channel < 4U));
 ...
 *                Refer @ref Cy_ERU_ETL_STATUS_FLAG_MODE_t for valid value.
 *
 * @return None
 *
 * \par<b>Description:</b><br>
 ...
 * during initialization in Cy_ERU_ETL_Init(). Call this to change the trigger edge, as needed later in the program.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_ClearStatusFlag(), Cy_ERU_ETL_GetStatusFlag()
 */
 ...
 *                Refer @ref Cy_ERU_ETL_OUTPUT_TRIGGER_CHANNEL_t for valid value.
 *
 * @return None
 *
 * \par<b>Description:</b><br>
 ...
 * based on the trigger pulse.If output trigger pulse generation is disabled by Cy_ERU_ETL_DisableOutputTrigger(),
 * Cy_ERU_ETL_EnableOutputTrigger() can called to reconfigure. These values are set during initialization in
 * Cy_ERU_ETL_Init(). Call this to change the trigger edge, as needed later in the program.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_DisableOutputTrigger()
 */
 ...
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_ETL_EnableOutputTrigger()
 */
 ...
 *               Refer data structure Cy_ERU_OGU_CONFIG_t for detail.
 *
 * @return None
 *
 * <b>Description:</b><br>
 ...
 *              Refer @ref Cy_ERU_OGU_PATTERN_DETECTION_INPUT_t for valid values. Logical <b>OR</b> combination of the
 ...
 * Cy_ERU_OGU_Init(). Call this to change the pattern, as needed later in the program.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_OGU_DisablePatternDetection(), Cy_ERU_OGU_GetPatternDetectionStatus()
 */
 ...
 * \par
 * Typically Cy_ERU_OGU_DisablePatternDetection is used when events has to be generated peripheral triggers.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_OGU_EnablePatternDetection(), Cy_ERU_OGU_GetPatternDetectionStatus()
 */
 ...
 * \par
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_OGU_EnablePatternDetection(), Cy_ERU_OGU_DisablePatternDetection()
 */
__STATIC_INLINE uint32_t Cy_ERU_OGU_GetPatternDetectionStatus(Cy_ERU_t *const eru,
    const uint8_t channel)
{
  CY_ASSERT(CY_ERU_CHECK_MODULE_PTR(eru));
  CY_ASSERT((channel < 4U));
 ...
 *                           Refer @ref Cy_ERU_OGU_PERIPHERAL_TRIGGER_t for the valid values, or
                             cy_eru_map.h file where the mapping of the peripheral input is done based
 ...
 * Cy_ERU_OGU_Init(). Call this to change the input, as needed later in the program.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_OGU_DisablePeripheralTrigger()
 */
 ...
 * Cy_ERU_OGU_DisablePeripheralTrigger(), Cy_ERU_OGU_EnablePeripheralTrigger() has to be called to reconfigure the
 * signals again.
 *
 * \par<b>Related APIs:</b><BR>
 * Cy_ERU_OGU_EnablePeripheralTrigger()
 */
 ...
 * initialization in Cy_ERU_OGU_Init(). Call this to change the gating mode, as needed later in the program.
 *
 */

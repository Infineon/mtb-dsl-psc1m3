 ...
 * MACROS
 ********************************************************************************************************************/
#define CY_VADC_MAX_ICLASS_SET          (2U)  /**< Defines the maximum number of conversion parameter sets */
#define CY_VADC_NUM_EMUX_INTERFACES     (2U)  /**< Defines the maximum number of external multiplexer  interfaces */

#define CY_VADC_RESULT_LEFT_ALIGN_10BIT (2U)  /**< Defines the 10 bit converted result register left align mask. It \
                                                    is used in the Cy_VADC_GLOBAL_SetCompareValue() API */

#define CY_VADC_SYNCTR_START_LOCATION (3U)  /**< Defines the location in SYNCTR needed for calculations*/
/*********************************************************************************************************************
 * ENUMS
 ********************************************************************************************************************/
...
static VADC_G_TypeDef *const g_cy_vadc_group_array[CY_VADC_MAXIMUM_NUM_GROUPS] = {(VADC_G_TypeDef *)(void *)VADC_G0,
...
void Cy_VADC_GLOBAL_EnableModule(void)
{
   ...
   * This hack is applicable only in particular the G11 step.
   *
   * Please refer to the PSC1M3 Errata sheet V1.0 to enable ADC
   * function
   */
  ...
  Cy_SCU_CLOCK_UngatePeripheralClock(CY_SCU_PERIPHERAL_CLOCK_VADC);
#endif

#if defined(CY_IP_PERIPHERAL_RESET)
  /* Reset the Hardware */
  Cy_SCU_RESET_DeassertPeripheralReset((Cy_SCU_PERIPHERAL_RESET_t)CY_SCU_PERIPHERAL_RESET_VADC );
#endif
}
...
#if defined(CY_IP_PERIPHERAL_RESET)
  /* Reset the Hardware */
  Cy_SCU_RESET_AssertPeripheralReset((Cy_SCU_PERIPHERAL_RESET_t)CY_SCU_PERIPHERAL_RESET_VADC );
#endif

#if defined(CY_IP_CLOCK_GATING)
  Cy_SCU_CLOCK_GatePeripheralClock(CY_SCU_PERIPHERAL_CLOCK_VADC);
#endif
...
void Cy_VADC_GLOBAL_Init(Cy_VADC_GLOBAL_t *const global_ptr, const Cy_VADC_GLOBAL_CONFIG_t *config)
{
  ...
  Cy_SCU_CLOCK_SetAdcClockSrc(CY_SCU_CLOCK_ADCCLKSRC_48MHZ);
                                    ...
                                    const Cy_VADC_GROUP_CONV_t conv_type, const uint32_t set_num)
{
  ...
  CY_ASSERT((global_ptr == VADC));
  CY_ASSERT(((conv_type) <= CY_VADC_GROUP_CONV_EMUX));
  CY_ASSERT((set_num < CY_VADC_MAX_ICLASS_SET));

#if(CY_VADC_EMUX_AVAILABLE == 1U)
  if (conv_type == CY_VADC_GROUP_CONV_STD )
  {
#endif
    CY_UNUSED_ARG(conv_type);
...
void Cy_VADC_GLOBAL_StartupCalibration(Cy_VADC_GLOBAL_t *const global_ptr)
{
  ...
  for (i = 0U; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    group_ptr = g_cy_vadc_group_array[i];
  ...
  while ((((SHS0->SHSCFG) & (uint32_t)SHS_SHSCFG_STATE_Msk) >> (uint32_t)SHS_SHSCFG_STATE_Pos) !=
         CY_VADC_SHS_START_UP_CAL_ACTIVE  )
  {
    __NOP();
  }
...
#if (CY_VADC_BOUNDARY_AVAILABLE == 1U)
void Cy_VADC_GLOBAL_SetBoundaries(Cy_VADC_GLOBAL_t *const global_ptr,
    ...
    const uint16_t boundary_value)
{
  ...
  CY_ASSERT((global_ptr == VADC));
  CY_ASSERT(((CY_VADC_CHANNEL_BOUNDARY_GLOBAL_BOUND0 == selection) ||
              (CY_VADC_CHANNEL_BOUNDARY_GLOBAL_BOUND1 == selection)));

  /* Program the Boundary registers */
  globbound = global_ptr->GLOBBOUND;

  if (CY_VADC_CHANNEL_BOUNDARY_GLOBAL_BOUND0 == selection)
  {
    globbound &= ~((uint32_t) VADC_GLOBBOUND_BOUNDARY0_Msk);
    globbound |= (uint32_t) ((uint32_t) boundary_value << VADC_GLOBBOUND_BOUNDARY0_Pos);
  }
  else if (CY_VADC_CHANNEL_BOUNDARY_GLOBAL_BOUND1 == selection)
  {
...
void Cy_VADC_GLOBAL_SetCompareValue(Cy_VADC_GLOBAL_t *const global_ptr, const Cy_VADC_RESULT_SIZE_t compare_val)
{
  ...
  global_ptr->GLOBRES |= (uint32_t)((uint32_t)compare_val << CY_VADC_RESULT_LEFT_ALIGN_10BIT);
}
...
Cy_VADC_FAST_COMPARE_t Cy_VADC_GLOBAL_GetCompareResult(Cy_VADC_GLOBAL_t *const global_ptr)
{
  ...
  {
    result = (Cy_VADC_FAST_COMPARE_t)((uint32_t)(res >> (uint32_t)VADC_GLOBRES_FCR_Pos) & (uint32_t)1);
  }
  else
  {
    result = CY_VADC_FAST_COMPARE_UNKNOWN;
  }
...
void Cy_VADC_GLOBAL_BindGroupToEMux(Cy_VADC_GLOBAL_t *const global_ptr, const uint32_t emuxif, const uint32_t group)
{
  ...
  CY_ASSERT((global_ptr == VADC));
  CY_ASSERT((emuxif < CY_VADC_NUM_EMUX_INTERFACES));
  CY_ASSERT((group < CY_VADC_MAXIMUM_NUM_GROUPS));
...
void Cy_VADC_GLOBAL_SetResultEventInterruptNode(Cy_VADC_GLOBAL_t *const global_ptr, Cy_VADC_SR_t sr)
{
  ...
  CY_ASSERT((global_ptr == VADC));
  CY_ASSERT((sr <= CY_VADC_SR_SHARED_SR3));

  if (sr >= CY_VADC_SR_SHARED_SR0)
  {
    node = (uint32_t)sr - (uint32_t)CY_VADC_SR_SHARED_SR0;
  }
  else
  {
    node = (uint32_t)sr;
...
void Cy_VADC_GROUP_Init( Cy_VADC_GROUP_t *const group_ptr, const Cy_VADC_GROUP_CONFIG_t *config)
{
  ...
  Cy_VADC_GROUP_InputClassInit(group_ptr, config->class0, CY_VADC_GROUP_CONV_STD, 0U);
  Cy_VADC_GROUP_InputClassInit(group_ptr, config->class0, CY_VADC_GROUP_CONV_EMUX, 0U);
  Cy_VADC_GROUP_InputClassInit(group_ptr, config->class1, CY_VADC_GROUP_CONV_STD, 1U);
  Cy_VADC_GROUP_InputClassInit(group_ptr, config->class1, CY_VADC_GROUP_CONV_EMUX, 1U);
  ...
  Cy_VADC_GROUP_ExternalMuxControlInit(group_ptr, config->emux_config);
                                   ...
                                   const Cy_VADC_GROUP_CONV_t conv_type, const uint32_t set_num)
{
   ...
   */
  if (CY_VADC_GROUP_CONV_STD == conv_type)
  {
    ...
    conv_mode        = (Cy_VADC_CONVMODE_t)config.conversion_mode_standard;
  }
  else
  {
    ...
    conv_mode        = (Cy_VADC_CONVMODE_t)config.conversion_mode_emux;
  }
...
void Cy_VADC_GROUP_SetPowerMode(Cy_VADC_GROUP_t *const group_ptr, const Cy_VADC_GROUP_POWERMODE_t power_mode)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT((power_mode <= CY_VADC_GROUP_POWERMODE_NORMAL));
...
void Cy_VADC_GROUP_SetSyncSlave(Cy_VADC_GROUP_t *const group_ptr, uint32_t master_grp, uint32_t slave_grp)
{
  uint32_t synctr;
#if (CY_VADC_MULTIPLE_SLAVEGROUPS == 1U )
#endif
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
...
void Cy_VADC_GROUP_CheckSlaveReadiness(Cy_VADC_GROUP_t *const group_ptr, uint32_t slave_group)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT((slave_group <= (CY_VADC_MAXIMUM_NUM_GROUPS - 1)));

  master_grp_num = 0;
  for (i = 0; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    if (g_cy_vadc_group_array[i] == group_ptr)
    {
      master_grp_num = i;
    }
  }
  ...
  group_ptr->SYNCTR |= (1U << (slave_group + CY_VADC_SYNCTR_START_LOCATION));
}
...
{
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT((eval_waiting_group != eval_origin_group ));

  if (eval_origin_group < eval_waiting_group)
  {
    eval_origin_group++;
  }
  group_ptr->SYNCTR |= (1U << (eval_origin_group + CY_VADC_SYNCTR_START_LOCATION));
}
...
Cy_VADC_GROUP_STATE_t Cy_VADC_GROUP_IsConverterBusy(Cy_VADC_GROUP_t *const group_ptr)
{
  ...
  return ( (Cy_VADC_GROUP_STATE_t)arbcfg);
}

/* API to set boundaries for conversion results */
void Cy_VADC_GROUP_SetBoundaries(Cy_VADC_GROUP_t *const group_ptr, const uint32_t boundary0, const uint32_t boundary1)
{
  uint32_t bound;
    ...
    const uint16_t boundary_value)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((CY_VADC_CHANNEL_BOUNDARY_GROUP_BOUND0 == selection) ||
              (CY_VADC_CHANNEL_BOUNDARY_GROUP_BOUND1 == selection)));

  /* Program the Boundary registers */
  bound  = group_ptr->BOUND;
  if (CY_VADC_CHANNEL_BOUNDARY_GROUP_BOUND0 == selection)
  {
    bound &= ~((uint32_t) VADC_G_BOUND_BOUNDARY0_Msk);
    bound |= (uint32_t) ((uint32_t) boundary_value << VADC_G_BOUND_BOUNDARY0_Pos);
  }
  else if (CY_VADC_CHANNEL_BOUNDARY_GROUP_BOUND1 == selection)
  {
    bound &= ~((uint32_t) VADC_G_BOUND_BOUNDARY1_Msk);
    ...
    const Cy_VADC_GROUP_IRQ_t type)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT((sr_num <= CY_VADC_SR_SHARED_SR3));
  CY_ASSERT(((type) <= CY_VADC_GROUP_IRQ_SHARED));

  sract = group_ptr->SRACT;

  if (CY_VADC_GROUP_IRQ_KERNEL == type)
  {
...
void Cy_VADC_GROUP_SetBoundaryEventInterruptNode(Cy_VADC_GROUP_t *const group_ptr,
    ...
    const Cy_VADC_BOUNDARY_NODE_t sr)
{
  uint32_t flag_pos;
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
...
void Cy_VADC_GLOBAL_SHS_Init(Cy_VADC_GLOBAL_SHS_t *const shs_ptr, const Cy_VADC_GLOBAL_SHS_CONFIG_t *config)
{
  ...
  CY_ASSERT((config == (Cy_VADC_GLOBAL_SHS_CONFIG_t *)NULL));
...
{
  CY_ASSERT((shs_ptr == (Cy_VADC_GLOBAL_SHS_t *)(void *)SHS0));
  CY_ASSERT((group_num <= CY_VADC_GROUP_INDEX_1));

  /* Set the converted to Accelerated mode from compatible mode*/
  if (group_num == CY_VADC_GROUP_INDEX_0 )
  {
    shs_ptr->TIMCFG0 |= (uint32_t)SHS_TIMCFG0_AT_Msk;
  }
  else if (group_num == CY_VADC_GROUP_INDEX_1 )
  {
...
{
  CY_ASSERT((shs_ptr == (Cy_VADC_GLOBAL_SHS_t *)(void *)SHS0));
  CY_ASSERT((group_num <= (uint32_t)CY_VADC_GROUP_INDEX_1));

  /* Enable gain and offset calibration*/
  if ( CY_VADC_GROUP_INDEX_0 == group_num)
  {
    shs_ptr->CALOC0 &= ~((uint32_t)SHS_CALOC0_DISCAL_Msk);
  }
  else if ( CY_VADC_GROUP_INDEX_1 == group_num)
  {
    ...
    Cy_VADC_SHS_GAIN_LEVEL_t gain_level)
{
  ...
  CY_ASSERT((shs_ptr == (Cy_VADC_GLOBAL_SHS_t *)(void *)SHS0));
  CY_ASSERT((group_num == CY_VADC_GROUP_INDEX_0) || (group_num == CY_VADC_GROUP_INDEX_1));
  CY_ASSERT((gain_level == CY_VADC_SHS_GAIN_LEVEL_0) || (gain_level == CY_VADC_SHS_GAIN_LEVEL_1) ||
             (gain_level == CY_VADC_SHS_GAIN_LEVEL_2) || (gain_level == CY_VADC_SHS_GAIN_LEVEL_3));
...
void Cy_VADC_GROUP_ScanInit(Cy_VADC_GROUP_t *const group_ptr, const Cy_VADC_SCAN_CONFIG_t *config)
{
  ...
  Cy_VADC_GROUP_ScanDisableArbitrationSlot(group_ptr);
  ...
  if (CY_VADC_STARTMODE_WFS != (Cy_VADC_STARTMODE_t)(config->conv_start_mode))
  {
  ...
  group_ptr->ASMR  = (uint32_t)((config->asmr) | (uint32_t)((uint32_t)CY_VADC_GATEMODE_IGNORE << VADC_G_ASMR_ENGT_Pos));

  if (CY_VADC_STARTMODE_CNR == (Cy_VADC_STARTMODE_t)(config->conv_start_mode))
  {
    group_ptr->ASMR |= (uint32_t)VADC_G_ASMR_RPTDIS_Msk;
  }

  /* Enable arbitration slot now */
  Cy_VADC_GROUP_ScanEnableArbitrationSlot(group_ptr);
...
void Cy_VADC_GROUP_ScanSelectTrigger(Cy_VADC_GROUP_t *const group_ptr, Cy_VADC_TRIGGER_INPUT_SELECT_t trigger_input)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((trigger_input) < CY_VADC_NUM_PORTS));
...
void Cy_VADC_GROUP_ScanSelectTriggerEdge(Cy_VADC_GROUP_t *const group_ptr, const Cy_VADC_TRIGGER_EDGE_t trigger_edge)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((trigger_edge) <= CY_VADC_TRIGGER_EDGE_ANY));
...
void Cy_VADC_GROUP_ScanSelectGating(Cy_VADC_GROUP_t *const group_ptr, Cy_VADC_GATE_INPUT_SELECT_t gating_input)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((gating_input) < CY_VADC_NUM_PORTS));
...
uint32_t Cy_VADC_GROUP_ScanGetNumChannelsPending(Cy_VADC_GROUP_t *const group_ptr)
{
  ...
  if (group_ptr->ASPND)
  {
    ...
    for (i = 0U; i < CY_VADC_NUM_CHANNELS_PER_GROUP; i++)
    {
      if (reg & 1U)
      {
        count++;
...
void Cy_VADC_GROUP_ScanRemoveChannel(Cy_VADC_GROUP_t *const group_ptr, const uint32_t channel_num)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((channel_num) < CY_VADC_NUM_CHANNELS_PER_GROUP));
...
void Cy_VADC_GLOBAL_BackgroundInit(Cy_VADC_GLOBAL_t *const global_ptr, const Cy_VADC_BACKGROUND_CONFIG_t *config)
{
...
#if (CY_VADC_GROUP_AVAILABLE ==1U)
  for (i = (uint8_t)0; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    Cy_VADC_GROUP_BackgroundDisableArbitrationSlot((Cy_VADC_GROUP_t *)g_cy_vadc_group_array[i]);
  }
  ...
  for (i = 0U; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    reg = g_cy_vadc_group_array[i]->ARBPR;
    ...
    g_cy_vadc_group_array[i]->ARBPR = reg;
  ...
  global_ptr->BRSMR = (uint32_t)((config->asmr) | (uint32_t)((uint32_t)CY_VADC_GATEMODE_IGNORE << VADC_BRSMR_ENGT_Pos));
  ...
  for (i = (uint8_t)0; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    Cy_VADC_GROUP_BackgroundEnableArbitrationSlot((Cy_VADC_GROUP_t *)g_cy_vadc_group_array[i]);
  }
#endif
...
void Cy_VADC_GLOBAL_BackgroundSelectTrigger(Cy_VADC_GLOBAL_t *const global_ptr, const uint32_t input_num)
{
  ...
  CY_ASSERT((global_ptr == VADC));
  CY_ASSERT(((input_num) < CY_VADC_NUM_PORTS));
...
/* Select a trigger edge*/
void Cy_VADC_GLOBAL_BackgroundSelectTriggerEdge(Cy_VADC_GLOBAL_t *const global_ptr,
    const Cy_VADC_TRIGGER_EDGE_t trigger_edge)
{
  uint32_t scanctrl;
...
void Cy_VADC_GLOBAL_BackgroundAbortSequence(Cy_VADC_GLOBAL_t *const global_ptr)
{
...
#if (CY_VADC_GROUP_AVAILABLE ==1U)
  uint32_t i;
  uint8_t grp_asen2_flag[CY_VADC_MAXIMUM_NUM_GROUPS];
#endif
  CY_ASSERT((global_ptr == VADC));
  ...
  for (i = (uint8_t)0; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    grp_asen2_flag[i] = (uint8_t)(g_cy_vadc_group_array[i]->ARBPR >> VADC_G_ARBPR_ASEN2_Pos);
    Cy_VADC_GROUP_BackgroundDisableArbitrationSlot((Cy_VADC_GROUP_t *)g_cy_vadc_group_array[i]);
  }
#endif
...
uint32_t Cy_VADC_GLOBAL_BackgroundGetNumChannelsPending(Cy_VADC_GLOBAL_t *const global_ptr)
{
  ...
  for (i = 0U; i < CY_VADC_MAXIMUM_NUM_GROUPS; i++)
  {
    if (global_ptr->BRSSEL[i])
    {
      ...
      for (j = 0U; j < CY_VADC_NUM_CHANNELS_PER_GROUP; j++)
      {
        if (reg & 1U)
        {
          count++;
...
void Cy_VADC_GROUP_QueueInit(Cy_VADC_GROUP_t *const group_ptr, const Cy_VADC_QUEUE_CONFIG_t *config)
{
  ...
  Cy_VADC_GROUP_QueueDisableArbitrationSlot(group_ptr);
  ...
  /* Gating mode */
  group_ptr->QMR0 = ((uint32_t)(config->qmr0) | (uint32_t)((uint32_t)CY_VADC_GATEMODE_IGNORE << VADC_G_QMR0_ENGT_Pos));
  ...
  Cy_VADC_GROUP_QueueEnableArbitrationSlot(group_ptr);
...
void Cy_VADC_GROUP_QueueAbortSequence(Cy_VADC_GROUP_t *const group_ptr)
{
  ...
  Cy_VADC_GROUP_QueueFlushEntries(group_ptr);
...
void Cy_VADC_GROUP_QueueRemoveChannel(Cy_VADC_GROUP_t *const group_ptr)
{
  ...
  length_before_abort = Cy_VADC_GROUP_QueueGetLength(group_ptr);
  ...
  if (length_before_abort)
  {
    ...
    length_after_abort = Cy_VADC_GROUP_QueueGetLength(group_ptr);
...
bool Cy_VADC_GROUP_ChannelIsResultOutOfBounds(Cy_VADC_GROUP_t *const group_ptr, const uint32_t ch_num)
{
  ...
  */
  ...
  if ( (( (uint32_t)CY_VADC_CHANNEL_EVGEN_INBOUND == chctr) \
        || ((uint32_t) CY_VADC_CHANNEL_EVGEN_OUTBOUND == chctr)) && (ceflag) )
  {
    retval = (bool)true;
  }
...
void Cy_VADC_GROUP_ChannelSetInputReference(Cy_VADC_GROUP_t *const group_ptr,
    ...
    const Cy_VADC_CHANNEL_REF_t ref)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((ch_num) < CY_VADC_NUM_CHANNELS_PER_GROUP));
  CY_ASSERT(((ref) <= CY_VADC_CHANNEL_REF_ALT_CH0));
    ...
    const uint32_t result_reg_num)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((ch_num) < CY_VADC_NUM_CHANNELS_PER_GROUP));
  CY_ASSERT(((result_reg_num) < CY_VADC_NUM_RESULT_REGISTERS));
                                     ...
                                     const Cy_VADC_CHANNEL_CONV_t conversion_class)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((ch_num) < CY_VADC_NUM_CHANNELS_PER_GROUP));
  CY_ASSERT((CY_VADC_CHANNEL_CONV_GLOBAL_CLASS1 >= conversion_class));
    ...
    const Cy_VADC_CHANNEL_EVGEN_t criteria)
{
  ...
  CY_ASSERT(CY_VADC_CHECK_GROUP_PTR(group_ptr));
  CY_ASSERT(((ch_num) < CY_VADC_NUM_CHANNELS_PER_GROUP));
  CY_ASSERT(((criteria) <= CY_VADC_CHANNEL_EVGEN_ALWAYS));
    ...
    const Cy_VADC_RESULT_SIZE_t compare_val)
{
  ...
  res |= (uint32_t)((uint32_t)compare_val << CY_VADC_RESULT_LEFT_ALIGN_10BIT);
...
Cy_VADC_FAST_COMPARE_t Cy_VADC_GROUP_GetFastCompareResult(Cy_VADC_GROUP_t *const group_ptr, const uint32_t res_reg)
{
  ...
  {
    result = (Cy_VADC_FAST_COMPARE_t)((uint32_t)(res >> (uint32_t)VADC_G_RES_FCR_Pos) & (uint32_t)1);
  }
  else
  {
    result = CY_VADC_FAST_COMPARE_UNKNOWN;
...
uint32_t Cy_VADC_GROUP_GetResultFifoTail(Cy_VADC_GROUP_t *const group_ptr, uint32_t res_reg)
{
  ...
  if ((bool)true == Cy_VADC_GROUP_IsResultRegisterFifoHead(group_ptr, res_reg))
  {
    res_reg = res_reg - 1U;
  }
...
uint32_t Cy_VADC_GROUP_GetResultFifoHead(Cy_VADC_GROUP_t *const group_ptr, const uint32_t res_reg)
{
  ...
  {
    head = CY_VADC_NUM_RESULT_REGISTERS - (uint32_t)1;

    for (i = res_reg; i < CY_VADC_NUM_RESULT_REGISTERS ; i++)
    {
      rcr = group_ptr->RCR[i];
      rcr &= (uint32_t)VADC_G_RCR_FEN_Msk;

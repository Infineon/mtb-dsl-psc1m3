...
# Default to hex file from the last configuration, but allow override by combiner-signer output or user specified PROG_FILE
_MTB_RECIPE__PROG_FILE_USER=$(_MTB_RECIPE__LAST_CONFIG_PROG_FILE)
...
_MTB_RECIPE__PROG_FILE_USER=$(MTB_COMBINE_SIGN_$(_MTB_RECIPE__COMBINE_SIGN_IDX)_HEX_PATH)
endif
...
ifeq ($(LIBNAME),)
	...
	$(MTB__NOISE)echo "$(_MTB_RECIPE__JLink_PROGRAM_COMMAND)"
	$(MTB__NOISE)$(_MTB_RECIPE__JLink_PROGRAM_COMMAND) || { \
		echo "Failed to program the $(DEVICE) device." \
			"Check if BMI (Boot Mode Index) is set" \
			"correctly. For more information, refer" \
			"to the $(or $(MTB_RECIPE__IDE),Eclipse)" \
			"for ModusToolbox user guide."; \
		exit 1; \
	}
	$(MTB__NOISE)echo "Programming complete"
else
...
erase_JLink:
	...
	$(MTB__NOISE)echo "$(_MTB_RECIPE__JLink_ERASE_COMMAND)"
	$(MTB__NOISE)$(_MTB_RECIPE__JLink_ERASE_COMMAND) || { \
		echo "Failed to erase the $(DEVICE) device." \
			"Check if BMI (Boot Mode Index) is set" \
			"correctly. For more information, refer" \
			"to the $(or $(MTB_RECIPE__IDE),Eclipse)" \
			"for ModusToolbox user guide."; \
